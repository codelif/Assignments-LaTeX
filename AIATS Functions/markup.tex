\documentclass[12pt,a4paper]{exam}
    ...
    \maketitle
      \begin{questions}
        ...
        \question{If $f(x)$ and $g(x)$ are two functions such that $f(x)=[x]+[-x]$ and $g(x)=\{x\}\forall \hspace{1mm} x \in R$ and $h(x)=f(g(x))$; then which of the following is incorrect? \\ ([$\cdot$] denotes greatest integer function and $\{\cdot\}$ denotes fractional part function)}
        \begin{flushleft}
          {(1) $f(x)$ and $h(x)$ are identical functions}\vspace{1mm}\break
          \vspace{1mm}{(2) $f(x) = g(x)$ has no solution}\break
          \vspace{1mm}{(3) $f(x)+h(x)>0$ has no solution}\break
          \vspace{1mm}{(4) $f(x)-h(x)$ is a periodic function}
        ...
        \question{The number of positive integral values of $x$ satisfying $\left[\frac{x}{9}\right]=\left[\frac{x}{11}\right]$ is:}
        \choice{21}{22}{23}{24}
        ...
        \question{The function $f(x)$ satisfy the equation $f(1-x)+2f(x)=3x \hspace{1mm} \forall \hspace{1mm} x\in R$, then $f(0)=$}
        \choice{$-2$}{$-1$}{0}{1}
